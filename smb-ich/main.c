...
static ich_dev_t* ich;

static void exit_signal(int signo)
{
    /* Release MMIO region */
    if (ich->base_mmio!=NULL)
    {
        munmap(ich->base_mmio, 256);
    }
...
static void start(ich_dev_t* dev)
{
    char                   name[PATH_MAX];
    struct sigaction       sa;
    resmgr_attr_t          rattr;
    ...
    iofunc_funcs_t         ocb_funcs = {_IOFUNC_NFUNCS, _ich_ocb_calloc, _ich_ocb_free};
...
int main(int argc, char* argv[])
{
    ...
    ich = dev = calloc(1, sizeof(ich_dev_t));
    ...
    if (ich_init(dev, 0) != 0)
    {
        fprintf(stderr, "[smb-ich] Initialization failed\n");
        free(dev);
        return -1;
    }
