...
int ec_devctl(resmgr_context_t* ctp, io_devctl_t* msg, ec_ocb_t* ocb)
{
    int         status, ret=EOK;
    ...
    {
        slogf(_SLOGC_EC_SMBUS, _SLOG_ERROR, "[smb-ec] iofunc_devctl_default() failed");
        return status;
    }
    ...
    switch (msg->i.dcmd)
    {
        case DCMD_SMBUS_READ:
             ret=ec_smbus_read(ctp, msg, ocb);
             break;
        case DCMD_SMBUS_WRITE:
             ret=ec_smbus_write(ctp, msg, ocb);
             break;
        default:
             slogf(_SLOGC_EC_SMBUS, _SLOG_ERROR, "[smb-ec] Unknown devctl command: %x", msg->i.dcmd);
             ret = ENOSYS;
             break;
    }
